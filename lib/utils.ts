...
export function getError(
	...
	const codeAndMessageText = `${typescript.DiagnosticCategory[
		info.category
	].toLowerCase()} TS${info.code}: ${typescript.flattenDiagnosticMessageText(
		info.messageText,
		"\n",
	)}`;
	...
	const startPos = typescript.getLineAndCharacterOfPosition(
		info.file,
		info.start,
	);
	const endPos = typescript.getLineAndCharacterOfPosition(
		info.file,
	...
	err.message =
		colors
			.red(`${fileName}(${startPos.line + 1},${startPos.character + 1}): `)
			.toString() + codeAndMessageText;
...
export function deprecate(
	...
	console.log(
		`  ${colors.gray(
			`More information: ${colors.underline(
