...
function defaultFinishHandler(results: CompilationResult) {
  ...
  const showErrorCount = (count: number, type: string) => {
    ...
    console.log(
      "TypeScript:",
      colors.magenta(count.toString()),
      (type !== "" ? `${type} ` : "") + (count === 1 ? "error" : "errors")
    );
    hasError = true;
  };
...
export function longReporter(): Reporter {
  return {
    error: (error: TypeScriptError, typescript: typeof ts) => {
      if (error.tsFile) {
        console.log(
          `[${colors.gray("gulp-typescript")}] ${colors.red(
            `${error.fullFilename}(${error.startPosition.line},${error.startPosition.character}): `
          )}error TS${
...
export function fullReporter(fullFilename: boolean = false): Reporter {
  return {
    error: (error: TypeScriptError, typescript: typeof ts) => {
      console.log(
        `[${colors.gray("gulp-typescript")}] ${colors.bgRed(
          `${error.diagnostic.code}`
        )} ${colors.red(
          typescript.flattenDiagnosticMessageText(
            error.diagnostic.messageText,
