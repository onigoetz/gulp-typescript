...
export class Output {
	...
	project: project.ProjectInfo;
	result: reporter.CompilationResult | undefined;
	// .js and .d.ts files
	streamFull: stream.Readable;
	// .js files
	streamJs: stream.Readable;
	...
	private async writeJsAsync(
		...
		await this.applySourceMap(sourceMapContent, original, file).then(
			(appliedSourceMap) => {
	...
	private async writeDtsAsync(
		...
		await this.applySourceMap(declarationMapContent, original, file).then(
			(appliedSourceMap) => {
				if (appliedSourceMap) file.sourceMap = JSON.parse(appliedSourceMap);
				this.streamFull.push(file);
	...
	private async applySourceMap(
		...
		const sourceMapOrigins = this.project.singleOutput
			? this.project.input
					.getFileNames(true)
					.map((fName) => this.project.input.getFile(fName))
			: [original];
	...
	private pipeRejection<T>(
		promise: Promise<T>,
		alternateStream: stream.Readable,
	) {
		promise.catch((err) => {
	...
	error(error: reporter.TypeScriptError) {
		...
		if (this.project.reporter.error)
			this.project.reporter.error(
				<reporter.TypeScriptError>error,
