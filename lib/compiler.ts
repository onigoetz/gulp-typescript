...
export class ProjectCompiler implements ICompiler {
  ...
  inputDone() {
    if (!this.project.input.firstSourceFile) {
      this.project.output.finish(
        emptyCompilationResult(this.project.options.noEmit)
      );
      return;
    }
    ...
    if (!this.project.singleOutput) {
      if (this.project.options.rootDir === undefined) {
        this.project.options.rootDir = utils.getCommonBasePathOfArray(
          rootFilenames
            .filter(fileName => fileName.substr(-5) !== ".d.ts")
    ...
    const currentDirectory = utils.getCommonBasePathOfArray(
      rootFilenames.map(
        fileName => this.project.input.getFile(fileName).gulp.cwd
      )
    );
    ...
    const preEmitDiagnostics: ReadonlyArray<ts.DiagnosticWithLocation> = [
      ...optionErrors,
      ...syntaxErrors,
      ...globalErrors,
      ...semanticErrors,
      ...
      this.emit(
        ...
        (fileName, content, writeByteOrderMark, onError, sourceFiles) => {
          ...
          if (sourceFiles.length !== 1) {
            throw new Error(
              `Failure: sourceFiles in WriteFileCallback should have length 1, got ${sourceFiles.length}`
            );
          }

          const fileNameOriginal = this.host.getCanonicalFileName(
  ...
  private attachContentToFile(
    ...
    const [, extension] = utils.splitExtension(fileName, ["d.ts", "d.ts.map", "d.mts", "d.mts.map", "d.cts", "d.cts.map"]);
    switch (extension) {
      case "cjs":
      case "mjs":
      case "js":
  ...
  private emit(
    ...
    preEmitDiagnostics: ReadonlyArray<ts.DiagnosticWithLocation>,
    callback: ts.WriteFileCallback
  ) {
  ...
  private emitFile(
    ...
    if (file) {
      ...
      if (this.project.options.declarationDir) {
        const baseRelative = path.relative(
          this.project.options.rootDir,
          file.gulp.base
        );
        baseDeclarations = path.join(
          this.project.options.declarationDir,
          baseRelative
        );
      }
    ...
    if (jsContent !== undefined) {
      ...
      this.project.output.writeJs(
        ...
        jsContent,
        jsMapContent,
        file ? file.gulp.cwd : currentDirectory,
        file
      );
    }
...
export class FileCompiler implements ICompiler {
  ...
  private write(
    ...
    this.output[file.fileNameNormalized] = {
      fileName,
      diagnostics,
      content,
      sourceMap
  ...
  inputFile(file: File) {
    if (
      file.fileNameNormalized.substr(file.fileNameNormalized.length - 5) ===
      ".d.ts"
    ) {
    ...
    if (
      this.project.input.getFileChange(file.fileNameOriginal).state ===
      FileChangeState.EQUAL
    ) {
    ...
    const output: ts.TranspileOutput = this.project.typescript.transpileModule(
      file.content,
      {
        compilerOptions: this.project.options,
        fileName: file.fileNameOriginal,
        reportDiagnostics: true,
        transformers: this.finalTransformers
          ? this.finalTransformers()
          : undefined
      }
    );
    ...
    if (mapString.substring(0, start.length) !== start) {
      console.log(
        "Couldn't read the sourceMap generated by TypeScript. This is likely an issue with gulp-typescript."
      );
      return;
    }
    ...
    const map: RawSourceMap = JSON.parse(
      Buffer.from(mapString, "base64").toString()
    );
    ...
    this.write(
      file,
      `${fileNameExtensionless}.${extension}`,
      output.diagnostics,
